...
public class AddClubCommandIntegrationTest {
    ...
    @Test
    public void execute_newClub_success() {
        Club validClub = new ClubBuilder().build();

        Model expectedModel = new ModelManager(model.getAddressBook(), new UserPrefs());
        expectedModel.addClub(validClub);

        assertCommandSuccess(new AddClubCommand(validClub), model,
                String.format(AddClubCommand.MESSAGE_SUCCESS, Messages.format(validClub)),
                expectedModel);
    }
